<?php
    ...
    define('DB_USER','csl');
    define('DB_PASS','e4ce2d9a415511e798fbb70e84c710C');
    define('DB_NAME','utils_db');
    define('DB_PORT',3358);
    define('DB_CHAR','utf8');
    define('APPNAME','');
    $conn=mysqli_connect(DB_HOST.':'.DB_PORT,DB_USER,DB_PASS);
    $conn->select_db(DB_NAME);
    $conn->query('set names ' . DB_CHAR);
...
<html xmlns="https://www.w3.org/1999/xhtml" lang="en" xml:lang="zh">
<head>
...
//查询数据库字段信息
        ...
        $tab_result=$conn->query($sql_tab);
        $tab_array=array();

        while($r=$tab_result->fetch_assoc()){
            $tab_array[]=$r;
        }
//show keys
        $keys_result=$conn->query("show keys from `".$array[$i]['Name'].'`');
        $arr_keys=$keys_result->fetch_array();
